...
async function searchBusinesses() {
  ...
  const maxScore = parseInt($('searchMaxScore')?.value, 10);
  ...
  try {
    const res = await fetch(API_BASE + '/api/search/businesses', {
      ...
      body: JSON.stringify({ query, location, radius: radiusKm * 1000, maxResults, portals, sort: sortBy, maxScore: maxScore < 100 ? maxScore : null, analyze: doAnalyze, discover: doDiscover }),
    });
